...
DirectoryEntry* AllocateEntry(unsigned long dir_cluster) {
    ...
    while (true) {
        ...
        auto next_cluster = NextCluster(dir_cluster);
    ...
    // クラスターを拡張して新しいクラスターの先頭のディレクトリエントリを返す
...
size_t FileDescriptor::Read(void* buf, size_t len) {
    ...
    size_t total = 0;
    while (total < len) {
        uint8_t* sector = GetSectorByCluster<uint8_t>(rd_cluster_);
        size_t n = std::min(len - total, bytes_per_cluster - rd_cluster_off_);
        memcpy(&buf8[total], &sector[rd_cluster_off_], n);
        total += n;

        rd_cluster_off_ += n;

        // クラスタの残りを読み終わったら次のクラスタへ（オフセットもリセット）
...
size_t FileDescriptor::Write(const void* buf, size_t len) {
    ...
    if (wr_cluster_ == 0) {
        if (fat_entry_.FirstCluster() != 0) {
            ...
            fat_entry_.first_cluster_low = wr_cluster_ & 0xffffu;
            fat_entry_.first_cluster_high = (wr_cluster_ >> 16) & 0xffffu;
        }
    }
    ...
    while (total < len) {
        ...
        uint8_t* sector = GetSectorByCluster<uint8_t>(wr_cluster_);
        size_t n = std::min(len, bytes_per_cluster - wr_cluster_off_);
        memcpy(&sector[wr_cluster_off_], &buf8[total], n);
        total += n;
        wr_cluster_off_ += n;
    }
