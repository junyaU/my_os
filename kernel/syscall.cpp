...
SYSCALL(PutString) {
    ...
    return {task.Files()[fd]->Write(s, len), EBADF};
}
...
Result DoWindowFunc(Func f, uint64_t layer_id_flags, Args... args) {
...
SYSCALL(WinRedraw) {
    return DoWindowFunc([](Window &) { return Result{0, 0}; }, arg1);
}
...
SYSCALL(WinDrawLine) {
    return DoWindowFunc(
        [](Window &win, int x0, int y0, int x1, int y1, uint32_t color) {
            ...
            if (dx == 0 && dy == 0) {
                win.Drawer()->Draw({x0, y0}, ToColor(color));
                return Result{0, 0};
            }
            ...
            if (abs(dx) >= abs(dy)) {
                ...
                for (int x = x0; x <= x1; x++) {
                    ...
                    win.Drawer()->Draw({x, y}, ToColor(color));
                }
            } else {
                if (dy < 0) {
                    std::swap(x0, x1);
        ...
        switch (msg->type) {
            case Message::kKeyPush:
                if (msg->arg.keyboard.keycode == 20 &&
                    ...
                    app_events[i].arg.keypush.modifier =
                        msg->arg.keyboard.modifier;
                    app_events[i].arg.keypush.keycode =
            ...
            case Message::kMouseMove:
                ...
                app_events[i].arg.mouse_move.buttons =
                    msg->arg.mouse_move.buttons;
                i++;
                break;
